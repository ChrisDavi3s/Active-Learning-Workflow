...
3) Generate VASP single-point input files (POSCAR, POTCAR, INCAR) for these “worst frames” (or any structures of interest) to obtain higher-fidelity reference data.

4) Submit these single-point jobs to a high-performance cluster using a Slurm job array or equivalent HPC script.

5) Gather the resulting final structures from VASP (e.g., from OUTCARs) and collect them into a single enhanced dataset (extxyz), which can then be used for model retraining or further analysis.
...
--------------------------------------------------------------------------------
# (3) generate_vasp_single_points.py
...
--------------------------------------------------------------------------------
# (4) HPC Submission Script (SCAN_zn_active.sh)
